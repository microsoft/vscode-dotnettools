# Install .NET with C# Doctor on macOS
...
**We require:** A new terminal must be able to find the intended `dotnet` host and the same SDK generation.

**Fix it:** Open a new terminal, inspect `which dotnet` and `dotnet --info`, and correct your shell startup files if another installation wins `PATH`.
